-- Sales Invoice GL preview alignment.
--
-- The posted Sales Invoice engine writes COGS and Inventory lines for
-- inventory items. This migration aligns the non-authoritative GL preview so
-- the workspace does not show the older AR/revenue/VAT-only model before a
-- document is posted.
...
BEGIN
  ...
  v_posting_date := COALESCE(p_posting_date, v_rec.date, CURRENT_DATE);
  ...
  RETURN jsonb_build_object(
    ...
    'source_route', CASE WHEN v_source_route IS NOT NULL
                         ...
                         ELSE NULL END,
    'rule_explanation',
      'Preview generated from the saved Sales Invoice. COGS and Inventory are estimated from current stock cost; posted journal lines remain authoritative.',
...
COMMENT ON FUNCTION fn_preview_sales_invoice_gl_impact(UUID, DATE) IS
  'Non-authoritative Sales Invoice GL preview. Includes estimated COGS and Inventory for inventory item lines without mutating stock or cost layers.';
