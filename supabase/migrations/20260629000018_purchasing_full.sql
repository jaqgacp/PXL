...
SELECT
  ...
  COALESCE((SELECT s.address_line1 FROM suppliers s WHERE s.id = vb.supplier_id), '') AS supplier_address,
  vb.supplier_invoice_number AS invoice_no,
  vb.bill_number        AS system_no,
...
SELECT
  ...
  COALESCE(ac.code, ac.atc_code) AS atc_code,
...
SELECT
  ...
  COALESCE((SELECT s.address_line1 FROM suppliers s WHERE s.id = vb.supplier_id), '') AS address,
  COALESCE(SUM(vbl.net_amount + vbl.input_vat_amount), 0) AS gross_purchases,
