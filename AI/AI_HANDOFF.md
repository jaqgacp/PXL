...
Under **PXL Accounting Core Ready** (DEC-017), session 100 completed the two remaining audit items. **PXL-AUD-045** added `20260714000010_aud045_si_expected_cwt_receipt_flow.sql` and `supabase/tests/053_si_expected_cwt_receipt_flow_test.sql`: SI expected CWT now stores the customer default ATC/base, validates expected CWT against ATC rate/base server-side, and OR prefill carries remaining SI expected CWT/base/ATC. **PXL-AUD-050** added reusable `AuditEvidenceBlock`, wired remaining core transaction pages, and added audit evidence to the generic Accounting Source drillback.
...
Under **PXL Accounting Core Ready** (DEC-017), session 98 completed **PXL-DA-018** with `supabase/migrations/20260714000009_da018_heavy_report_readiness.sql` and `supabase/tests/052_heavy_report_readiness_test.sql` (HEAVY-REPORT-READINESS-001, 18 assertions). The migration adds GL report indexes plus `fn_general_ledger_report`, `fn_gl_account_ledger_summary`, `fn_gl_account_ledger_page`, and `fn_trial_balance_report`. General Ledger, Account Detail Ledger, and Trial Balance now use those RPCs for bounded rows, total row counts, full-period debit/credit totals, opening/closing/running balances, and TB entry-class mode aggregation instead of broad client-side `vw_general_ledger` materialization.

Verification completed for session 98: held-out-safe `supabase db reset --local --no-seed` passed with held-out drafts `20260710000004`/`00005` moved aside/restored; focused test 052 passed 18/18; full trusted pgTAP passed **900/900 across 51 files** with held-out test 027 excluded. `npm run gen:types` and schema summary regenerated (272 functions / 20 views / 149 tables / 287 triggers; 111 migrations / 52 tests). `npm run lint`, `npm run build`, and `git diff --check` passed. Hosted Supabase was pushed through `20260714000009` in session 100.

DA-018 is now **Retested Passed** and was pushed with session 100. Sessions 99-100 completed DA-020 plus AUD-045/AUD-050.

## Prior Priority (session 97 - DA-013 as-of ledger reconciliation)

Under **PXL Accounting Core Ready** (DEC-017), session 97 completed **PXL-DA-013** with `supabase/migrations/20260714000008_da013_asof_ledger_reconciliation.sql` and `supabase/tests/051_asof_ledger_reconciliation_test.sql` (ASOF-LEDGER-RECON-001, 16 assertions). The migration adds `fn_customer_ledger_asof`, `fn_supplier_ledger_asof`, `fn_ar_subledger_gl_reconciliation_asof`, and `fn_ap_subledger_gl_reconciliation_asof`. Customer ledgers are cutoff-aware and include posted SIs, invoice-applied OR rows, applied CMs, and paid DMs while excluding customer advances from AR. Supplier ledgers include posted VBs net of source-accrued EWT, PV bill applications, and posted VCs while excluding supplier down payments from AP.

Verification completed for session 97: held-out-safe `supabase db reset --local` passed with held-out drafts `20260710000004`/`00005` moved aside/restored; focused test 051 passed 16/16; full trusted pgTAP passed **882/882 across 50 files** with held-out test 027 excluded. `npm run gen:types` and schema summary regenerated (266 functions / 20 views / 149 tables / 287 triggers; 110 migrations / 51 tests). `npm run lint`, `npm run build`, and `bash -n scripts/gen_schema_summary.sh scripts/check_docs_consistency.sh` passed. Hosted Supabase push applied `20260714000006`, `20260714000007`, and `20260714000008` to linked project `bskjkogijpbhukjkagfj` with the held-out draft migrations moved aside/restored; a held-out-safe post-push dry run returned "Remote database is up to date." The CLI emitted the known pg-delta catalog-cache warning after apply, but migration history verified clean through the dry run.

DA-013 is now **Retested Passed** and hosted-synced. Sessions 98-100 completed DA-018, DA-020, AUD-045, and AUD-050.

## Prior Priority (session 96 - accounting-readiness approval/posting closure)

Under **PXL Accounting Core Ready** (DEC-017), session 96 completed **PXL-AUD-009/PXL-AUD-010** with `supabase/migrations/20260714000007_aud009_aud010_accounting_readiness_closure.sql` and `supabase/tests/050_accounting_readiness_approval_test.sql` (ACCOUNTING-READINESS-APPROVAL-001, 17 assertions). The migration adds AP supplier-TIN snapshot defaulting for VB/PV, blocks VB source-basis EWT approval when the supplier TIN snapshot remains blank, and blocks PV EWT posting when the supplier TIN snapshot remains blank. The test closes the SI/VB seeded evidence gap for missing account, inactive account, inactive VAT code, direct status transitions, valid approve/post, and AP-side EWT missing-TIN cases.

Verification completed for session 96: held-out-safe `supabase db reset --local --no-seed` passed with held-out drafts `20260710000004`/`00005` moved aside/restored; focused test 050 passed 17/17; full trusted pgTAP passed **866/866 across 49 files** with held-out test 027 excluded. `npm run gen:types` and schema summary regenerated (262 functions / 20 views / 149 tables / 287 triggers; 109 migrations / 50 tests). `npm run lint`, `npm run build`, `bash -n scripts/gen_schema_summary.sh`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase push for `20260714000006` through `20260714000008` completed in session 97.

AUD-009 and AUD-010 are now **Retested Passed** locally.

## Prior Priority (session 95 - supplier/customer withholding defaults)

Under **PXL Accounting Core Ready** (DEC-017), session 95 completed **PXL-AUD-008** with `supabase/tests/049_withholding_master_defaults_test.sql` (WHT-MASTER-DEFAULTS-001, 15 assertions). No schema migration was needed: the test closes the remaining evidence gap over existing ATC-backed defaults by proving invalid FWT defaults are rejected, valid supplier/customer defaults auto-enable withholding flags, supplier defaults drive source-basis VB EWT/tax detail/issued Form 2307 lines, and customer defaults drive receipt CWT/tax detail/received Form 2307 evidence.

Verification completed for session 95: held-out-safe `supabase db reset --local --no-seed` passed with held-out drafts `20260710000004`/`00005` moved aside/restored; focused test 049 passed 15/15; full trusted pgTAP passed **849/849 across 48 files** with held-out test 027 excluded. `npm run gen:types` and schema summary regenerated (261 functions / 20 views / 149 tables / 285 triggers; 108 migrations / 49 tests). `npm run lint`, `npm run build`, `bash -n scripts/gen_schema_summary.sh`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase push for `20260714000006` through `20260714000008` completed in session 97.

AUD-008 is now **Retested Passed** locally.

## Prior Priority (session 94 - withholding drilldowns)

Under **PXL Accounting Core Ready** (DEC-017), session 94 completed **PXL-AUD-049** with `supabase/migrations/20260714000006_aud049_withholding_trace_drilldowns.sql` + `supabase/tests/048_withholding_trace_drilldowns_test.sql` (WITHHOLDING-TRACE-DRILLDOWN-001, 9 assertions). `fn_get_report_trace_set` now accepts source-document, ATC, nature, tax-rate, and active-only filters so CV/CP/cash-sale EWT/CWT amounts and QAP/Form 2307 issued rows resolve to exact tax-ledger/source-document traces. `CheckVouchersPage`, `CashPurchasesPage`, `CashSalesPage`, `QAPPage`, and `Form2307IssuedPage` now link visible withholding/report-line amounts through the PXL-DA-002 trace contract.

Verification completed for session 94: held-out-safe `supabase db reset --local --no-seed` passed with held-out drafts `20260710000004`/`00005` moved aside/restored; focused test 048 passed 9/9; full trusted pgTAP passed **834/834 across 47 files** with held-out test 027 excluded. `npm run gen:types` and schema summary regenerated (261 functions / 20 views / 149 tables / 285 triggers; 108 migrations / 48 tests). `npm run lint`, `npm run build`, `bash -n scripts/gen_schema_summary.sh`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase push for `20260714000006` through `20260714000008` completed in session 97.

AUD-049 is now **Retested Passed** locally.

## Prior Priority (session 93 - Form 2307 received claim lifecycle)

Under **PXL Accounting Core Ready** (DEC-017), session 93 completed **PXL-AUD-047** with `supabase/migrations/20260714000005_form2307_received_claim_lifecycle.sql` + `supabase/tests/047_form2307_received_claim_lifecycle_test.sql` (FORM2307-RECEIVED-CLAIM-001, 18 assertions). Received Form 2307 evidence now routes through `fn_record_form2307_received`, claims route through `fn_claim_form2307_received`, certificate amounts validate against receipt-line and unreversed-ledger CWT, claimed/invalidated rows are locked, `fn_bounce_receipt` invalidates linked evidence after reversing CWT tax detail, and issued certificates are flagged for supersede when source EWT reverses after sent/acknowledged status.

Verification completed for session 93: held-out-safe `supabase db reset --local`; focused test 047 passed 18/18; neighbor tests 001/007/010/012/016/021/022/044/046/047 passed 184/184; full trusted pgTAP passed **825/825 across 46 files** with held-out test 027 excluded; `npm run gen:types` and schema summary regenerated (261 functions / 20 views / 149 tables / 285 triggers); `npm run lint`, `npm run build`, `bash -n scripts/gen_schema_summary.sh`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase push applied `20260714000003`, `20260714000004`, and `20260714000005`; a held-out-safe post-push dry run returned "Remote database is up to date." The CLI emitted the known pg-delta catalog-cache warning after apply, but migration history verified clean through the dry run.

AUD-047 is now **Retested Passed**.

## Prior Priority (session 92 - cash-sale receipt total semantics)
...
AUD-046 is now **Retested Passed**.
...
## Prior Priority (session 89 - advance/down-payment withholding)

Under **PXL Accounting Core Ready** (DEC-017), session 89 completed **PXL-AUD-043** with `supabase/migrations/20260714000001_advance_payment_withholding.sql` + `supabase/tests/043_advance_payment_withholding_test.sql` (ADVANCE-PAYMENT-WHT-001, 13 assertions). Official Receipts now support explicit customer-advance lines with no SI, CWT ATC/base/amount validation, customer advances clearing-account posting, source-line CWT tax detail, WHT reconciliation, setup/readiness wiring, GL preview, and UI add-line controls. Payment Vouchers now support explicit supplier down-payment lines with no VB, EWT profile gating, EWT ATC/base/amount validation, supplier down-payments asset posting, source-line EWT tax detail, WHT reconciliation, setup/readiness wiring, GL preview, and UI add-line controls. Later application of posted advances/down-payments to subsequently issued SI/VB remains a separate AR/AP settlement enhancement, not an open withholding-recording defect.
...
**Note — earlier state was stale:** sessions 78–84 (not captured in the prior session-77 snapshot) already closed **AUD-041**, the **AUD-034** residue, **DA-009**, **DA-019**, **AUD-037**, and **AUD-042** via migrations `20260713000005`–`20260713000011`. The authoritative status is the Findings Status Index in `docs/PXL/PXL_END_TO_END_AUDIT_FINDINGS.md`: **72 Retested Passed / 0 In Progress / 0 Open (72), no Criticals open**. Session 86 closed **PXL-AUD-016** and **PXL-AUD-013 + PXL-DA-014** (`20260713000013`, test 040). Session 87 closed **PXL-DA-016** (`20260713000014`, test 041). Sessions 88-89 closed **PXL-AUD-043** (`20260713000015`, test 042; `20260714000001`, test 043). Session 90 closed **PXL-AUD-040** (`20260714000002`, test 044). Session 91 closed **PXL-AUD-044** (`20260714000003`, test 045). Session 92 closed **PXL-AUD-046** (`20260714000004`, test 046). Session 93 closed **PXL-AUD-047** (`20260714000005`, test 047). Session 94 closed **PXL-AUD-049** (`20260714000006`, test 048). Session 95 closed **PXL-AUD-008** (test 049). Session 96 closed **PXL-AUD-009/PXL-AUD-010** (`20260714000007`, test 050). Session 97 closed **PXL-DA-013** (`20260714000008`, test 051). Session 98 closed **PXL-DA-018** (`20260714000009`, test 052). Session 99 closed **PXL-DA-020** (docs-consistency matrix sync gate). Session 100 closed **PXL-AUD-045/PXL-AUD-050** (`20260714000010`, test 053 plus audit UI rollout).
...
**Current next fix:** no audit findings remain open; continue AIQ-017 only under the core-readiness gates.
