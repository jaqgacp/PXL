...
- Findings standing: 27 Retested Passed / 13 In Progress / 30 Open (70 findings); 12 Criticals remain (AUD-002, AUD-006, AUD-031, AUD-032, AUD-033, AUD-034, DA-001, DA-002, DA-004, DA-008, DA-009, DA-019). Sessions 47 and 48 were documentation-only; no accounting/tax/posting behavior changed.
...
Continue `AIQ-008` with PXL-AUD-031 (add explicit VAT-exclusive `cwt_tax_base` + variance path to receipt lines, mirroring the PV design; update `fn_validate_receipt_line_cwt`, `fn_post_receipt`, `fn_save_cash_sale`, `ReceiptsPage`; implement scenario CWT-NET-BASE-001). Then PXL-AUD-032+033 together (check-voucher EWT validation/supplier linkage + counter-row cancel; scenario CV-EWT-2307-001), then PXL-AUD-034 (1601EQ reconciliation gate). Remember: `npm run gen:types` after every migration; backfills on non-draft rows need the replica-role escape hatch.
