...
## Active Priority (session 89 - advance/down-payment withholding)

Under **PXL Accounting Core Ready** (DEC-017), session 89 completed **PXL-AUD-043** with `supabase/migrations/20260714000001_advance_payment_withholding.sql` + `supabase/tests/043_advance_payment_withholding_test.sql` (ADVANCE-PAYMENT-WHT-001, 13 assertions). Official Receipts now support explicit customer-advance lines with no SI, CWT ATC/base/amount validation, customer advances clearing-account posting, source-line CWT tax detail, WHT reconciliation, setup/readiness wiring, GL preview, and UI add-line controls. Payment Vouchers now support explicit supplier down-payment lines with no VB, EWT profile gating, EWT ATC/base/amount validation, supplier down-payments asset posting, source-line EWT tax detail, WHT reconciliation, setup/readiness wiring, GL preview, and UI add-line controls.

Verification completed for session 89: held-out-safe `supabase db reset --local`; focused test 043 passed 13/13; neighbor tests 034/036/037/038/042/043 passed 76/76; full trusted pgTAP passed **774/774 across 42 files** with held-out test 027 excluded; `npm run gen:types` and schema summary regenerated (254 functions / 20 views / 152 tables / 287 triggers); `npm run lint`, `npm run build`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase sync for `20260714000001` is pending; hosted remains synced through `20260713000015`.

AUD-043 is now **Retested Passed**. Later application of posted advances/down-payments to subsequently issued SI/VB remains a separate AR/AP settlement enhancement, not an open withholding-recording defect.
...
**Next fix:** continue with PXL-AUD-040 (per-month Form 2307 breakdown), then AUD-044/046/047/049 and remaining In-Progress coverage/report items.
