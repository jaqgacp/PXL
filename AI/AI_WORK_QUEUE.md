...
AIQ-008 (continue): PXL-DA-011, PXL-AUD-005, PXL-DA-015, PXL-DA-017, PXL-AUD-029, PXL-AUD-030, PXL-AUD-031, PXL-AUD-032+033, and PXL-AUD-034 are closed — do not redo them. Next: the EWT High pair PXL-AUD-035+036 (ATC as-of-document-date validation + rate versioning; ATC-ASOF-001 / ATC-RATE-VERSION-001), or resume the pre-existing Critical lane with PXL-DA-001 (server-side GL preview RPC). Pre-existing Criticals PXL-DA-001/002/004/008/009/019 and AUD-002/006 remain. Regenerate `src/lib/database.types.ts` (`npm run gen:types`) after every migration; backfills on non-draft rows of DA-011-guarded tables need `session_replication_role = replica` (`tax_detail_entries` is NOT guarded; `ewt_returns` is, and now also carries the reconciliation gate). Summary doc AIQ-007 follows when audit work pauses.
