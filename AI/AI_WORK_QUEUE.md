...
| AIQ-008 | P0 | In Progress | Audit | Continue the highest-severity open audit finding from `docs/PXL/PXL_END_TO_END_AUDIT_FINDINGS.md`. Repeat until no open findings remain. | Audit findings, test book, transaction matrix | Finding is fixed or marked blocked with evidence; tests/docs updated. Sessions 27 (PXL-AUD-026 grants), 28 (PXL-AUD-027 tax ledger void counter-entries), 30 (PXL-DA-003/PXL-AUD-004 `fn_can_perform` per DEC-009), and 31 (PXL-DA-012 approval SoD per DEC-010) landed. |
...
AIQ-008 (continue): PXL-AUD-014 VAT ledger completeness (zero-VAT rows, classification bases, cash sales/purchases writers; prerequisites documented in session 28), or PXL-DA-017 dimension propagation per DEC-011. Summary docs AIQ-006–007 follow when audit work pauses.
