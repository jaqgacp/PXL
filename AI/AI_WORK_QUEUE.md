...
AIQ-008 (continue): PXL-DA-011, PXL-AUD-005, PXL-DA-015, PXL-DA-017, PXL-AUD-029, PXL-AUD-030, PXL-AUD-031, and PXL-AUD-032+033 are closed — do not redo them. Next: PXL-AUD-034 (1601EQ reconciliation gate mirroring the VAT-return gate; pairs with PXL-AUD-041's remittance flow). Pre-existing Criticals PXL-DA-001/002/004/008/009/019 and AUD-002/006 remain after that. Regenerate `src/lib/database.types.ts` (`npm run gen:types`) after every migration; backfills on non-draft rows of DA-011-guarded tables need `session_replication_role = replica` (`tax_detail_entries` is NOT guarded). Summary doc AIQ-007 follows when audit work pauses.
