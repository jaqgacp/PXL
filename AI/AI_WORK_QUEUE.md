...
| AIQ-008 | P0 | In Progress | Audit | Continue highest-severity open audit findings until none remain. | Audit findings, test book, transaction matrix | Current finding fixed or genuinely blocked; tests/docs/hosted state updated. Current standing: **58 passed / 8 in progress / 6 open; no Criticals open**. Sessions 86-87 closed AUD-016, AUD-013 + DA-014, and DA-016; close work is committed as `38ac28a`, DA-016 is implemented in `20260713000014` + test 041, trusted pgTAP is **751/751 across 40 files**, and hosted Supabase is synced through `20260713000014` with held-out drafts excluded/restored checksum-clean. |
...
Standing (authoritative = Findings Status Index): **58 Retested Passed / 8 In Progress / 6 Open (72), no Criticals open.** Session 86 closed **PXL-AUD-016** and **PXL-AUD-013 + PXL-DA-014** (`20260713000013`, test 040), committed as `38ac28a`. Session 87 closed **PXL-DA-016** with governed `transaction_events` lifecycle evidence (`20260713000014`, test 041, 14 assertions); trusted pgTAP is **751/751 across 40 files** and hosted Supabase is synced through `20260713000014`.

Next unblocked core work: **PXL-AUD-043** (EWT on cash purchases / advances), then **PXL-AUD-040** (per-month 2307 breakdown), **PXL-AUD-046**/**047** (cash-sale total + 2307-claim correctness), and the remaining In-Progress test/coverage/report items (AUD-008/009/010, DA-013/018/020, AUD-044/045/049/050). Close follow-ups deferred to backlog: a Period-Close UI over `fn_close_fiscal_year`, an optional Income-Summary close, and configurable FS-line mappings.
