...
**AIQ-015 is the active sole priority (DEC-015; final SI brief 2026-07-13).** The final dense Sales Invoice viewing/lifecycle template, saved-view table framework, and official canonical workspace standard are implemented. Next is **P5B**: relocate draft create/edit + new-document creation into the same route/shell, retire the register form, then add the missing schema-backed master-data links and register actions/columns. Only after the SI pilot is genuinely complete should P6 roll the documented template to Vendor Bill and the other transaction families. AIQ-008's remaining Criticals (**PXL-DA-009**, **PXL-DA-019**) remain paused.
