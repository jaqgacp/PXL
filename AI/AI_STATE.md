...
Hosted Supabase is synced through `20260713000015`; local migrations `20260714000001_advance_payment_withholding.sql` and `20260714000002_form2307_monthly_breakdown.sql` are pending hosted sync. The CLI project-ref form is unsupported in this workspace, so pushes use the linked project. During hosted pushes and trusted resets, the two tracked held-out draft migrations are moved aside and restored checksum-clean:
...
- Session 90: `20260714000002_form2307_monthly_breakdown.sql` + `supabase/tests/044_form2307_monthly_breakdown_test.sql` (F2307-MONTHLY-001, 8 assertions) passed. Held-out-safe `supabase db reset --local` passed; neighbor tests 007/010/016/022/044 passed 75/75; full trusted pgTAP passed **782/782 across 43 files** with held-out test 027 excluded. `npm run gen:types` and trusted-baseline schema summary regenerated (254 functions / 20 views / 152 tables / 287 triggers). `npm run lint`, `npm run build`, `scripts/check_docs_consistency.sh`, `git diff --check`, and `git diff --cached --check` passed. Hosted Supabase sync is pending for `20260714000001` and `20260714000002`.
