...
export default function TaxableIncomeComputationPage() {
  ...
  const load = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('income_tax_computations').select('*').eq('company_id', companyId).eq('deduction_method', 'itemized')
  ...
  const set = (k: keyof FormData, v: string | number) => setForm(f => {
    const next = { ...f, [k]: v }
    if (k === 'gross_income' || k === 'allowable_deductions' || k === 'tax_rate') {
      next.taxable_income = Math.max(next.gross_income - next.allowable_deductions, 0)
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    const payload = { company_id: companyId, deduction_method: 'itemized', ...form, period_quarter: form.period_type === 'annual' ? null : form.period_quarter }
    if (!editId) {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← Taxable Income Computation</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Taxable Income Computation' : editId ? 'Edit Computation' : 'New Computation'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">{fmtPeriod(form)} — Itemized Deductions</p>
          </div>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Allowable Deductions (Itemized)</label>{isView ? <div className={ro}>{fmtNum(form.allowable_deductions)}</div> : <input type="number" step="0.01" value={form.allowable_deductions} onChange={e => set('allowable_deductions', Number(e.target.value))} className={inp} />}</div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">Taxable Income Computation</h1><p className="text-sm text-gray-500 mt-0.5">Gross Income less Itemized Deductions</p></div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-16 text-gray-400"><p className="text-base font-medium text-gray-500">No Computations Found</p><p className="text-sm mt-1 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'Click "+ New Computation" to begin.'}</p></td></tr>
              ) : rows.map((r, i) => (
