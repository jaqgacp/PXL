...
export default function PTWorkingPapersPage() {
  ...
  const loadHeaders = async () => {
    ...
    const { data } = await supabase
      .from('compliance_pt_working_papers_headers')
      .select('*')
  ...
  const loadLines = async (headerId: string): Promise<FormLine[]> => {
    const { data } = await supabase.from('compliance_pt_working_papers_lines').select('*').eq('header_id', headerId).order('created_at')
  ...
  const openEdit = async (h: Header) => {
    setForm({ period_year: h.period_year, period_quarter: h.period_quarter, description: h.description || '', status: h.status })
    setLines(await loadLines(h.id)); setEditId(h.id); setMode('edit')
  }

  const openView = async (h: Header) => {
    setForm({ period_year: h.period_year, period_quarter: h.period_quarter, description: h.description || '', status: h.status })
    setLines(await loadLines(h.id)); setEditId(h.id); setMode('view')
  }
  ...
  const handleSave = async () => {
    ...
    if (!headerId) {
      const { data, error } = await supabase.from('compliance_pt_working_papers_headers')
        ...
        .select('id').single()
      if (error) {
        alert('Cannot save PT Working Paper.\nReason: ' + (error.code === '23505' ? `A working paper for ${fmtQuarter(form.period_year, form.period_quarter)} already exists.` : error.message))
        setSaving(false); return
      }
      headerId = data.id
    } else {
      const { error } = await supabase.from('compliance_pt_working_papers_headers')
        .update({ period_year: form.period_year, period_quarter: form.period_quarter, description: form.description || null, status: form.status })
        .eq('id', headerId)
      if (error) { alert('Cannot update PT Working Paper.\nReason: ' + error.message); setSaving(false); return }
    }

    if (editId) {
      const keepIds = lines.filter(l => l.id).map(l => l.id!)
      const { data: dbLines } = await supabase.from('compliance_pt_working_papers_lines').select('id').eq('header_id', headerId)
      const toDelete = ((dbLines || []) as { id: string }[]).filter(dl => !keepIds.includes(dl.id)).map(dl => dl.id)
      if (toDelete.length > 0) await supabase.from('compliance_pt_working_papers_lines').delete().in('id', toDelete)
    }

    for (const l of lines) {
      if (l.id) {
        await supabase.from('compliance_pt_working_papers_lines').update({ reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }).eq('id', l.id)
      } else {
        await supabase.from('compliance_pt_working_papers_lines').insert([{ header_id: headerId, reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }])
      }
    }
  ...
  const filtered = headers.filter(h => {
    ...
    const matchSearch = !q || fmtQuarter(h.period_year, h.period_quarter).toLowerCase().includes(q) || (h.description || '').toLowerCase().includes(q)
    const matchStatus = filterStatus === 'all' || h.status === filterStatus
    const matchYear = !filterYear || h.period_year === filterYear
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← PT Working Papers</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'PT Working Paper' : editId ? 'Edit PT Working Paper' : 'New PT Working Paper'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">Percentage Tax — {fmtQuarter(form.period_year, form.period_quarter)}</p>
          </div>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className={lbl}>Quarter <span className="text-red-500">*</span></label>
            ...
            <div className="col-span-3">
              ...
              {isView ? <textarea readOnly value={form.description || '—'} rows={2} className={ro} /> : (
                <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2}
                  placeholder="e.g. PT Working Paper for Q1 2026 — exempt/zero-rated sales schedule" className={inp} />
              )}
            </div>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {lines.length === 0 ? (
                  <tr><td colSpan={isView ? 4 : 5} className="text-center py-12 text-xs text-gray-400">
                    {isView ? 'No line items on this working paper.' : 'No lines yet. Click "+ Add Row" to begin.'}
                  </td></tr>
                ) : lines.map((l, i) => (
                  <tr key={l._key} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-2">
                      {isView ? <span className="text-sm text-gray-700">{l.reference || '—'}</span> : (
                        <input value={l.reference} onChange={e => setLineField(l._key, 'reference', e.target.value)}
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900" placeholder="Reference / Transaction ID" />
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2">
                      {isView ? <span className="text-sm text-gray-700">{l.remarks || '—'}</span> : (
                        <input value={l.remarks} onChange={e => setLineField(l._key, 'remarks', e.target.value)}
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900" placeholder="Notes" />
                      )}
                    </td>
                    {!isView && <td className="px-2 py-2 text-center">
                      <button onClick={() => removeLine(l._key)} className="text-gray-300 hover:text-red-500 text-sm font-medium transition-colors" aria-label="Remove line">✕</button>
                    </td>}
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">PT Working Papers</h1>
          <p className="text-sm text-gray-500 mt-0.5">Percentage Tax — Schedule &amp; Reconciliation</p>
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-gray-900" placeholder="Search quarter or description..." />
        <select value={filterYear} onChange={e => setFilterYear(e.target.value ? Number(e.target.value) : '')} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none">
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="px-4 py-3 flex gap-4 animate-pulse">
                <div className="h-3 bg-gray-100 rounded w-28" /><div className="h-3 bg-gray-100 rounded flex-1" /><div className="h-3 bg-gray-100 rounded w-16" />
              </div>
            ))}
          </div>
        ) : (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-gray-400">
                  <p className="text-base font-medium text-gray-500">No PT Working Papers Found</p>
        ...
        {filtered.length > 0 && <div className="px-4 py-3 border-t border-gray-100 text-xs text-gray-500">Showing {filtered.length} of {headers.length} working paper{headers.length !== 1 ? 's' : ''}</div>}
      </div>
    </div>
  )
}
