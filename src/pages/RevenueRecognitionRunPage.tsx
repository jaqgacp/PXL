...
export default function RevenueRecognitionRunPage() {
  ...
  return (
    <LegacyTransactionWorkspace title="Revenue Recognition Run" family="journal" pattern="D" posting
      documentNo={asOfDate} status={showResults ? 'posted' : 'draft'} identity="Due revenue-recognition entries"
      ...
      sourceDocType="REVREC" sourceDocId={previewEntryId}
      actions={[
        { key: 'refresh', label: loading ? 'Refreshing…' : 'Refresh', onClick: load, disabled: !companyId || loading },
        { key: 'post', label: running ? 'Running…' : `Recognize All (${entries.length})`, onClick: runAll, disabled: running || entries.length === 0, variant: 'primary' },
      ]}
      headerFields={[
        ...
        { key: 'basis', label: 'Recognition Basis', card: 1, span: 2, content: <div className="pxl-readonly-field">Due revenue recognition schedule entries</div> },
        { key: 'scope', label: 'Run Scope', card: 2, span: 2, content: <div className="pxl-readonly-field">{entries.length} pending entries</div> },
      ]}
      tabContent={{
        validation: <div className="space-y-2">{error && <div className="pxl-validation-message border border-red-200 bg-red-50 text-red-700">{error}</div>}<div className="pxl-validation-message border border-gray-200">{entries.length ? `${entries.length} entries are ready for recognition.` : 'No entries are currently due.'}</div></div>,
