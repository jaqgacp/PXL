...
const FORM_TYPE = '1702RT'
const now = new Date()
const EMPTY_FORM: FormData = { period_year: now.getFullYear(), gross_income: 0, taxable_income: 0, tax_due: 0, tax_credits: 0, tax_payable: 0, status: 'draft', filed_date: '', reference_no: '', remarks: '' }
...
export default function ITR1702RTPage() {
  ...
  const load = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('itr_filings').select('*').eq('company_id', companyId).eq('form_type', FORM_TYPE).order('period_year', { ascending: false })
    setRows((data as Row[]) || [])
    setLoading(false)
  }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    const payload = { company_id: companyId, form_type: FORM_TYPE, period_quarter: null, ...form, filed_date: form.filed_date || null, reference_no: form.reference_no || null, remarks: form.remarks || null }
    if (!editId) {
      const { error } = await supabase.from('itr_filings').insert([payload])
      if (error) { alert('Cannot save.\nReason: ' + (error.code === '23505' ? `A 1702RT filing for FY ${form.period_year} already exists.` : error.message)); setSaving(false); return }
    } else {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← 1702RT Annual ITR</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Annual ITR — 1702RT' : editId ? 'Edit Filing' : 'New Filing'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">FY {form.period_year} — Regular Rate</p>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">1702RT Annual ITR</h1><p className="text-sm text-gray-500 mt-0.5">Annual Income Tax Return — Corporate (Regular Rate)</p></div>
