...
const now = new Date()
const EMPTY_FORM: FormData = { period_year: now.getFullYear(), period_quarter: Math.floor(now.getMonth() / 3) + 1, total_tax_base: 0, total_fwt_withheld: 0, remitted_prior: 0, still_due: 0, status: 'draft', filed_date: '', reference_no: '', remarks: '' }
...
export default function FWT1601FQReturnPage() {
  ...
  const load = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('fwt_returns').select('*').eq('company_id', companyId).order('period_year', { ascending: false }).order('period_quarter', { ascending: false })
  ...
  const handleGenerate = async () => {
    if (!companyId) return
    setGenerating(true)
    const { data } = await supabase.from('form_2306_issuances').select('gross_interest_income,fwt_withheld')
      .eq('company_id', companyId).eq('period_year', form.period_year).eq('period_quarter', form.period_quarter)
    const rows = (data || []) as { gross_interest_income: number; fwt_withheld: number }[]
    const taxBase = rows.reduce((s, r) => s + Number(r.gross_interest_income), 0)
    const withheld = rows.reduce((s, r) => s + Number(r.fwt_withheld), 0)
    setForm(f => ({ ...f, total_tax_base: taxBase, total_fwt_withheld: withheld, still_due: withheld - f.remitted_prior }))
    setGenerating(false)
  }
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      total_tax_base: form.total_tax_base, total_fwt_withheld: form.total_fwt_withheld,
      remitted_prior: form.remitted_prior, still_due: form.total_fwt_withheld - form.remitted_prior,
      status: form.status, filed_date: form.filed_date || null, reference_no: form.reference_no || null, remarks: form.remarks || null,
    }
    if (!editId) {
      const { error } = await supabase.from('fwt_returns').insert([payload])
      if (error) { alert('Cannot save.\nReason: ' + (error.code === '23505' ? `A 1601FQ return for ${fmtQuarter(form.period_year, form.period_quarter)} already exists.` : error.message)); setSaving(false); return }
    } else {
      const { error } = await supabase.from('fwt_returns').update(payload).eq('id', editId)
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← 1601FQ Quarterly Return</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Quarterly FWT Return — 1601FQ' : editId ? 'Edit Return' : 'New Return'}</h1>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>Total Tax Base (Gross Interest)</label>{isView ? <div className={ro}>{fmtNum(form.total_tax_base)}</div> : <input type="number" step="0.01" value={form.total_tax_base} onChange={e => set('total_tax_base', Number(e.target.value))} className={inp} />}</div>
            <div><label className={lbl}>Total FWT Withheld</label>{isView ? <div className={ro}>{fmtNum(form.total_fwt_withheld)}</div> : <input type="number" step="0.01" value={form.total_fwt_withheld} onChange={e => set('total_fwt_withheld', Number(e.target.value))} className={inp} />}</div>
            <div><label className={lbl}>Less: Remitted Prior (0619-F)</label>{isView ? <div className={ro}>{fmtNum(form.remitted_prior)}</div> : <input type="number" step="0.01" value={form.remitted_prior} onChange={e => set('remitted_prior', Number(e.target.value))} className={inp} />}</div>
            <div className="col-span-2 pt-2 border-t border-gray-100"><label className={lbl}>Still Due</label><div className="text-2xl font-bold font-mono tabular-nums text-gray-900">{fmtNum(form.total_fwt_withheld - form.remitted_prior)}</div></div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">1601FQ Quarterly Return</h1><p className="text-sm text-gray-500 mt-0.5">Quarterly Final Withholding Tax Return</p></div>
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">FWT Withheld</th>
            ...
            <tbody>
              {returns.length === 0 ? (
                ...
                <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-right font-mono tabular-nums text-gray-700">{fmtNum(r.total_fwt_withheld)}</td>
