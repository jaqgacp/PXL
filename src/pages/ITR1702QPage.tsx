...
export default function ITR1702QPage() {
  ...
  const handleSave = async () => {
    ...
    if (!editId) {
      ...
      if (error) { alert('Cannot save.\nReason: ' + (error.code === '23505' ? `A 1702Q filing for ${fmtQuarter(form.period_year, form.period_quarter)} already exists.` : error.message)); setSaving(false); return }
    } else {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← 1702Q Quarterly ITR</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Quarterly ITR — 1702Q' : editId ? 'Edit Filing' : 'New Filing'}</h1>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">1702Q Quarterly ITR</h1><p className="text-sm text-gray-500 mt-0.5">Quarterly Income Tax Return — Corporate</p></div>
