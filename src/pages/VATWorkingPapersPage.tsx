...
type Header = { id: string; company_id: string; period: string; description: string | null; status: Status; created_at: string; updated_at: string }
...
type FormData = { period: string; description: string; status: Status }
...
const fmtPeriod = (d: string) => { if (!d) return '—'; const [y, m] = d.split('-'); return `${MONTHS[parseInt(m) - 1]} ${y}` }
...
export default function VATWorkingPapersPage() {
  ...
  const loadHeaders = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('compliance_vat_working_papers_headers').select('*').eq('company_id', companyId).order('period', { ascending: false })
  ...
  const loadLines = async (headerId: string): Promise<FormLine[]> => {
    const { data } = await supabase.from('compliance_vat_working_papers_lines').select('*').eq('header_id', headerId).order('created_at')
  ...
  const openNew = () => {
    ...
    setForm({ ...EMPTY_FORM, period: `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-01` })
    setLines([]); setEditId(null); setMode('new')
  }

  const openEdit = async (h: Header) => { setForm({ period: h.period, description: h.description || '', status: h.status }); setLines(await loadLines(h.id)); setEditId(h.id); setMode('edit') }
  const openView = async (h: Header) => { setForm({ period: h.period, description: h.description || '', status: h.status }); setLines(await loadLines(h.id)); setEditId(h.id); setMode('view') }
  ...
  const handleSave = async () => {
    if (!companyId || !form.period) { alert('Cannot save.\nReason: Period is required.'); return }
    setSaving(true)
    let headerId = editId

    if (!headerId) {
      const { data, error } = await supabase.from('compliance_vat_working_papers_headers')
        .insert([{ company_id: companyId, period: form.period, description: form.description || null, status: form.status }]).select('id').single()
      if (error) {
        alert('Cannot save VAT Working Paper.\nReason: ' + (error.code === '23505' ? `A working paper for ${fmtPeriod(form.period)} already exists.` : error.message))
        setSaving(false); return
      }
      headerId = data.id
    } else {
      const { error } = await supabase.from('compliance_vat_working_papers_headers')
        .update({ period: form.period, description: form.description || null, status: form.status }).eq('id', headerId)
      if (error) { alert('Cannot update VAT Working Paper.\nReason: ' + error.message); setSaving(false); return }
    }

    if (editId) {
      const keepIds = lines.filter(l => l.id).map(l => l.id!)
      const { data: dbLines } = await supabase.from('compliance_vat_working_papers_lines').select('id').eq('header_id', headerId)
      const toDelete = ((dbLines || []) as { id: string }[]).filter(dl => !keepIds.includes(dl.id)).map(dl => dl.id)
      if (toDelete.length > 0) await supabase.from('compliance_vat_working_papers_lines').delete().in('id', toDelete)
    }

    for (const l of lines) {
      if (l.id) await supabase.from('compliance_vat_working_papers_lines').update({ reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }).eq('id', l.id)
      else await supabase.from('compliance_vat_working_papers_lines').insert([{ header_id: headerId, reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }])
    }
  ...
  const filtered = headers.filter(h => {
    ...
    const matchSearch = !q || fmtPeriod(h.period).toLowerCase().includes(q) || (h.description || '').toLowerCase().includes(q)
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← VAT Working Papers</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'VAT Working Paper' : editId ? 'Edit VAT Working Paper' : 'New VAT Working Paper'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">Value-Added Tax — {fmtPeriod(form.period) || 'No period selected'}</p>
          </div>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="month" value={form.period ? form.period.slice(0, 7) : ''} onChange={e => set('period', e.target.value ? e.target.value + '-01' : '')} readOnly={isView} className={isView ? ro : inp} />
            </div>
            <div>
              <label className={lbl}>Status</label>
              {isView ? <div className="mt-1.5"><StatusBadge status={form.status} /></div> : (
                <select value={form.status} onChange={e => set('status', e.target.value as Status)} className={inp}>
                  <option value="draft">Draft</option><option value="final">Final</option><option value="filed">Filed</option>
                </select>
              )}
            </div>
            <div className="col-span-2">
              <label className={lbl}>Description</label>
              {isView ? <textarea readOnly value={form.description || '—'} rows={2} className={ro} /> : (
                <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2} placeholder="e.g. VAT Working Paper for June 2026 — Output/Input reconciliation" className={inp} />
              )}
            </div>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {lines.length === 0 ? (
                  <tr><td colSpan={isView ? 4 : 5} className="text-center py-12 text-xs text-gray-400">{isView ? 'No line items on this working paper.' : 'No lines yet. Click "+ Add Row" to begin.'}</td></tr>
                ) : lines.map((l, i) => (
                  <tr key={l._key} className="hover:bg-gray-50/50">
                    <td className="px-4 py-2 text-xs text-gray-400">{i + 1}</td>
                    <td className="px-4 py-2">
                      {isView ? <span className="text-sm text-gray-700">{l.reference || '—'}</span> : (
                        <input value={l.reference} onChange={e => setLineField(l._key, 'reference', e.target.value)} className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900" placeholder="Reference / Transaction ID" />
                      )}
                    </td>
                    <td className="px-4 py-2 text-right"><span className="text-sm font-mono tabular-nums text-gray-700">{fmtNum(l.amount)}</span></td>
                    <td className="px-4 py-2">
                      {isView ? <span className="text-sm text-gray-700">{l.remarks || '—'}</span> : (
                        <input value={l.remarks} onChange={e => setLineField(l._key, 'remarks', e.target.value)} className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900" placeholder="Notes" />
                      )}
                    </td>
                    {!isView && <td className="px-2 py-2 text-center"><button onClick={() => removeLine(l._key)} className="text-gray-300 hover:text-red-500 text-sm font-medium transition-colors" aria-label="Remove line">✕</button></td>}
                  </tr>
                ))}
              </tbody>
              {lines.length > 0 && (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">VAT Working Papers</h1>
          <p className="text-sm text-gray-500 mt-0.5">Value-Added Tax — Schedule &amp; Reconciliation</p>
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-gray-900" placeholder="Search period or description..." />
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as typeof filterStatus)} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none">
          <option value="all">All Status</option><option value="draft">Draft</option><option value="final">Final</option><option value="filed">Filed</option>
        </select>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-gray-400">
                  <p className="text-base font-medium text-gray-500">No VAT Working Papers Found</p>
                  <p className="text-sm mt-1 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'Click "+ New Working Paper" to create the first schedule.'}</p>
                </td></tr>
              ) : filtered.map((h, i) => (
                <tr key={h.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => openView(h)} className="text-xs text-gray-500 hover:text-gray-700 font-medium">View</button>
