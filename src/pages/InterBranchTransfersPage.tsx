...
export default function InterBranchTransfersPage() {
  ...
  // IBT allocates its number series against the source (from) branch — preflight the same branch.
  ...
  const post = async (id: string) => {
    ...
    try {
      const { error: previewError } = await supabase.rpc('fn_preview_gl_impact', { p_source_doc_type: 'IBT', p_source_doc_id: id })
      if (previewError) throw new Error(`Inter-Branch Transfer is not ready to post: ${previewError.message}`)
      const { error: e } = await supabase.rpc('fn_post_inter_branch_transfer', { p_ibt_id: id })
      if (e) throw e
      await load(); setMode('list')
