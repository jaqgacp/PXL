...
export default function BooksCashDisbursementsPage() {
  ...
  const exportCSV = () => {
    ...
    const csvRows = rows.map(r => [r.date, DOC_LABEL[r.doc_type], r.doc_number, r.payee, r.amount.toFixed(2)])
    const csv = [header, ...csvRows].map(row => row.map(c => `"${c}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `cash-disbursements-book-${dateFrom}-to-${dateTo}.csv`; a.click()
