...
export default function FWTWorkingPapersPage() {
  ...
  const loadHeaders = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('compliance_fwt_working_papers_headers').select('*').eq('company_id', companyId).order('period', { ascending: false })
  ...
  const loadLines = async (headerId: string): Promise<FormLine[]> => {
    const { data } = await supabase.from('compliance_fwt_working_papers_lines').select('*').eq('header_id', headerId).order('created_at')
  ...
  const openNew = () => { const t = new Date(); setForm({ ...EMPTY_FORM, period: `${t.getFullYear()}-${String(t.getMonth() + 1).padStart(2, '0')}-01` }); setLines([]); setEditId(null); setMode('new') }
  ...
  const handleSave = async () => {
    ...
    let headerId = editId
    if (!headerId) {
      const { data, error } = await supabase.from('compliance_fwt_working_papers_headers')
        .insert([{ company_id: companyId, period: form.period, description: form.description || null, status: form.status }]).select('id').single()
      if (error) { alert('Cannot save.\nReason: ' + (error.code === '23505' ? `A working paper for ${fmtPeriod(form.period)} already exists.` : error.message)); setSaving(false); return }
      headerId = data.id
    } else {
      const { error } = await supabase.from('compliance_fwt_working_papers_headers').update({ period: form.period, description: form.description || null, status: form.status }).eq('id', headerId)
      if (error) { alert('Cannot update.\nReason: ' + error.message); setSaving(false); return }
    }
    if (editId) {
      const keepIds = lines.filter(l => l.id).map(l => l.id!)
      const { data: dbLines } = await supabase.from('compliance_fwt_working_papers_lines').select('id').eq('header_id', headerId)
      const toDelete = ((dbLines || []) as { id: string }[]).filter(dl => !keepIds.includes(dl.id)).map(dl => dl.id)
      if (toDelete.length > 0) await supabase.from('compliance_fwt_working_papers_lines').delete().in('id', toDelete)
    }
    for (const l of lines) {
      if (l.id) await supabase.from('compliance_fwt_working_papers_lines').update({ reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }).eq('id', l.id)
      else await supabase.from('compliance_fwt_working_papers_lines').insert([{ header_id: headerId, reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }])
    }
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← FWT Working Papers</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'FWT Working Paper' : editId ? 'Edit Working Paper' : 'New Working Paper'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">Final Withholding Tax — {fmtPeriod(form.period) || 'No period selected'}</p>
          </div>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>Schedule Period <span className="text-red-500">*</span></label><input type="month" value={form.period ? form.period.slice(0, 7) : ''} onChange={e => set('period', e.target.value ? e.target.value + '-01' : '')} readOnly={isView} className={isView ? ro : inp} /></div>
            <div><label className={lbl}>Status</label>{isView ? <div className="mt-1.5"><StatusBadge status={form.status} /></div> : <select value={form.status} onChange={e => set('status', e.target.value as Status)} className={inp}><option value="draft">Draft</option><option value="final">Final</option><option value="filed">Filed</option></select>}</div>
            <div className="col-span-2"><label className={lbl}>Description</label>{isView ? <textarea readOnly value={form.description || '—'} rows={2} className={ro} /> : <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2} placeholder="e.g. FWT Working Paper for June 2026 — bank interest income" className={inp} />}</div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">FWT Working Papers</h1><p className="text-sm text-gray-500 mt-0.5">Final Withholding Tax — Schedule &amp; Reconciliation</p></div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {headers.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-gray-400"><p className="text-base font-medium text-gray-500">No FWT Working Papers Found</p><p className="text-sm mt-1 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'Click "+ New Working Paper" to create the first schedule.'}</p></td></tr>
              ) : headers.map((h, i) => (
