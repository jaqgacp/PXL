...
export default function BooksSalesJournalPage() {
  ...
  const run = useCallback(async () => {
    ...
    const { data } = await supabase.from('sales_invoices')
      ...
      .eq('company_id', companyId).eq('status', 'posted').eq('is_cash_sale', false)
  ...
  const exportCSV = () => {
    const header = ['Date', 'SI No.', 'Customer', 'TIN', 'VAT', 'Total Amount']
    const csvRows = rows.map(r => [r.date, r.si_number, r.customer_name_snapshot, r.customer_tin_snapshot || '', r.total_vat_amount.toFixed(2), r.total_amount.toFixed(2)])
    const csv = [header, ...csvRows].map(row => row.map(c => `"${c}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `sales-journal-${dateFrom}-to-${dateTo}.csv`; a.click()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Sales Journal</h1>
          <p className="text-sm text-gray-500 mt-0.5">Credit (on-account) sales invoices — BIR Book of Accounts</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-16 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'No credit sales in this period.'}</td></tr>
              ) : rows.map(r => (
