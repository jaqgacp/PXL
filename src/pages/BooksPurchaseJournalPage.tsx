...
type Row = { id: string; bill_date: string; bill_number: string; supplier_name_snapshot: string; supplier_tin_snapshot: string | null; total_input_vat_amount: number; total_amount: number }

const fmt = (n: number) => new Intl.NumberFormat('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n)
const today = () => new Date().toISOString().split('T')[0]
const firstOfMonth = () => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-01` }

export default function BooksPurchaseJournalPage() {
  ...
  const run = useCallback(async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('vendor_bills')
      .select('id,bill_date,bill_number,supplier_name_snapshot,supplier_tin_snapshot,total_input_vat_amount,total_amount')
      .eq('company_id', companyId).eq('status', 'posted')
      .gte('bill_date', dateFrom).lte('bill_date', dateTo).order('bill_date').order('bill_number')
    setRows((data as Row[]) || [])
    setLoading(false)
  ...
  const exportCSV = async () => {
    if (!companyId) return
    setExporting(true)
    const fileName = `purchase-journal-${dateFrom}-to-${dateTo}.csv`
    ...
    const header = ['Date', 'VB No.', 'Supplier', 'TIN', 'Input VAT', 'Total Amount']
    const csvRows = frozen.map(r => [str(r.bill_date), str(r.bill_number), str(r.supplier_name), str(r.supplier_tin), num(r.total_input_vat_amount), num(r.total_amount)])
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Purchase Journal</h1>
          <p className="text-sm text-gray-500 mt-0.5">Credit (on-account) vendor bills — BIR Book of Accounts</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">VB No.</th>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-16 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'No credit purchases in this period.'}</td></tr>
              ) : rows.map(r => (
                <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-2 text-gray-700">{r.bill_date}</td>
                  <td className="px-4 py-2 text-gray-700 font-mono">{r.bill_number}</td>
            ...
            {rows.length > 0 && (
              <tfoot className="border-t-2 border-gray-300 bg-gray-50">
                <tr>
                  <td colSpan={4} className="px-4 py-2.5 text-xs font-semibold text-gray-600 uppercase tracking-wide">Total — {rows.length} bill{rows.length !== 1 ? 's' : ''}</td>
