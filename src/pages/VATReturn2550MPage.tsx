...
const EMPTY_FORM: FormData = {
  ...
  net_vat_payable: 0, status: 'draft', filed_date: '', reference_no: '', remarks: '',
}
...
const fmtPeriod = (y: number, m: number) => `${MONTHS[m - 1]} ${y}`
...
export default function VATReturn2550MPage() {
  ...
  const load = async () => {
    ...
    const [{ data: company }, { data }] = await Promise.all([
      ...
      supabase.from('vat_returns').select('*').eq('company_id', companyId).eq('return_type', '2550M')
  ...
  const openNew = () => {
    if (!isVatRegistered) {
      alert('Cannot create VAT Return.\nReason: 2550M is only available for VAT-registered companies.')
      return
    }
    setForm({ ...EMPTY_FORM }); setEditId(null); setMode('new')
  }
  const openEdit = (r: ReturnRow) => {
    setForm({ ...r, filed_date: r.filed_date || '', reference_no: r.reference_no || '', remarks: '' })
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true)
    const startDate = `${form.period_year}-${String(form.period_month).padStart(2, '0')}-01`
    const endDate = new Date(form.period_year, form.period_month, 0).toISOString().split('T')[0]

    const [{ data: outData }, { data: inData }, { data: priorReturns }] = await Promise.all([
      ...
      supabase.from('vat_returns').select('period_year,period_month,net_vat_payable').eq('company_id', companyId).eq('return_type', '2550M')
        .order('period_year', { ascending: false }).order('period_month', { ascending: false }),
    ])

    const prevReturn = ((priorReturns || []) as { period_year: number; period_month: number; net_vat_payable: number }[])
      .find(r => r.period_year < form.period_year || (r.period_year === form.period_year && r.period_month < form.period_month))
    ...
    const carriedOver = prevReturn && Number(prevReturn.net_vat_payable) < 0 ? -Number(prevReturn.net_vat_payable) : 0
    const totalAvailableInputVat = inputVat + carriedOver
    ...
    setForm(f => ({
      ...
      input_taxable_purchases: inputTaxable, input_vat: inputVat, input_vat_carried_over: carriedOver,
      total_available_input_vat: totalAvailableInputVat, net_vat_payable: netPayable,
    }))
    setGenerating(false)
  }
  ...
  const handleSave = async () => {
    ...
    if (!isVatRegistered) { alert('Cannot save VAT Return.\nReason: 2550M is only available for VAT-registered companies.'); return }
    setSaving(true)
    const payload = {
      company_id: companyId, return_type: '2550M',
      period_year: form.period_year, period_month: form.period_month, period_quarter: null,
      ...
      net_vat_payable: form.net_vat_payable, vat_paid_prior_months: 0,
      vat_still_due: Math.max(form.net_vat_payable, 0),
    ...
    if (!editId) {
      ...
      if (error) { alert('Cannot save VAT Return.\nReason: ' + (error.code === '23505' ? `A 2550M return for ${fmtPeriod(form.period_year, form.period_month)} already exists.` : error.message)); setSaving(false); return }
    } else {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← VAT Return 2550M</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Monthly VAT Declaration — 2550M' : editId ? 'Edit VAT Return' : 'New VAT Return'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">{fmtPeriod(form.period_year, form.period_month)}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              {isView ? <div className={ro}>{form.period_year}</div> : (
                <select value={form.period_year} onChange={e => set('period_year', Number(e.target.value))} className={inp}>{years.map(y => <option key={y} value={y}>{y}</option>)}</select>
              )}
            </div>
            <div>
              <label className={lbl}>Month</label>
              {isView ? <div className={ro}>{MONTHS[form.period_month - 1]}</div> : (
                <select value={form.period_month} onChange={e => set('period_month', Number(e.target.value))} className={inp}>{MONTHS.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}</select>
              )}
            </div>
            <div>
        ...
        <div className={sec}>
          <h2 className={hd}>Sales / Output VAT</h2>
        ...
        <div className={sec}>
          <h2 className={hd}>Purchases / Input VAT</h2>
          <div className="grid grid-cols-2 gap-4">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">VAT Return — 2550M</h1>
          <p className="text-sm text-gray-500 mt-0.5">Monthly VAT Declaration</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Net Payable</th>
            ...
            <tbody>
              {returns.length === 0 ? (
                ...
                <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                  <td className="px-4 py-3 font-medium text-gray-900">{fmtPeriod(r.period_year, r.period_month)}</td>
                  ...
                  <td className="px-4 py-3 text-right font-mono tabular-nums text-gray-900 font-semibold">{fmtNum(r.net_vat_payable)}</td>
