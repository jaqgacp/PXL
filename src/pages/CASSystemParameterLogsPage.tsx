...
const TABLE_LABELS: Record<string, string> = {
  number_series: 'Number Series', approval_workflows: 'Approval Workflow', sys_feature_enablement: 'Feature Enablement',
  compliance_profiles: 'Compliance Profile', tax_codes: 'Tax Code', vat_codes: 'VAT Code', atc_codes: 'ATC Code',
}

export default function CASSystemParameterLogsPage() {
  ...
  const fetchLogs = async () => {
    if (!companyId) return
    setLoading(true)
    let query = supabase.from('sys_audit_logs').select('*')
  ...
  useEffect(() => { if (companyId) fetchLogs() }, [companyId, filterTable, page])

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">System Parameter Logs</h1>
        <p className="text-sm text-gray-500 mt-0.5">Change history for number series, approval workflows, feature flags &amp; tax configuration</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">Parameter</th>
            ...
            <tbody>
              {logs.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'No system parameter changes recorded.'}</td></tr>
              ) : logs.map(l => (
                <Fragment key={l.id}>
