...
type Schedule = {
  ...
  asset_account_id: string; expense_account_id: string
...
const EMPTY_FORM: FormState = {
  schedule_name: '', description: '', asset_account_id: '', expense_account_id: '',
...
export default function AmortizationSchedulesPage() {
  ...
  const load = useCallback(async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('amortization_schedules').select('*')
  ...
  const save = async () => {
    ...
    if (!form.schedule_name || !form.asset_account_id || !form.expense_account_id) {
      setError('Schedule name, asset account, and expense account are required'); return
    }
  ...
  const cancelSchedule = async (id: string) => {
    ...
    const { error: e } = await supabase.rpc('fn_cancel_amortization_schedule', { p_schedule_id: id })
    if (e) { setError(e.message); return }
    await load()
    if (viewSchedule?.id === id) { const s = schedules.find(x => x.id === id); if (s) loadEntries({ ...s, status: 'cancelled' }) }
  }
  ...
  if (mode === 'view' && viewSchedule) {
    const assetAcc = accounts.find(a => a.id === viewSchedule.asset_account_id)
    const expAcc = accounts.find(a => a.id === viewSchedule.expense_account_id)
    const pendingCount = entries.filter(e => e.status === 'pending').length
    const progress = viewSchedule.total_periods > 0 ? (viewSchedule.posted_periods / viewSchedule.total_periods) * 100 : 0

    return (
      <LegacyTransactionWorkspace title="Amortization Schedule" family="journal" pattern="D" posting
        documentNo={viewSchedule.schedule_name} status={viewSchedule.status} identity={viewSchedule.description}
        financialFacts={[{ label: 'Schedule Amount', value: fmt(viewSchedule.total_amount) }, { label: 'Posted Periods', value: `${viewSchedule.posted_periods} / ${viewSchedule.total_periods}` }, { label: 'Pending Periods', value: pendingCount }]}
        contextFacts={[{ label: 'Schedule', value: viewSchedule.schedule_name }, { label: 'Asset / Prepaid Account', value: assetAcc ? `${assetAcc.account_code} — ${assetAcc.account_name}` : viewSchedule.asset_account_id }, { label: 'Expense Account', value: expAcc ? `${expAcc.account_code} — ${expAcc.account_name}` : viewSchedule.expense_account_id }, { label: 'Progress', value: `${Math.round(progress)}%` }]}
        sourceDocType="AMORT" sourceDocId={previewEntryId || viewSchedule.id} auditTable="amortization_schedules"
        onBack={() => { setMode('list'); setViewSchedule(null); setEntries([]) }} backLabel="Amortization Schedules"
        actions={[
        ...
        headerFields={[
          ...
          { key: 'asset', label: 'Asset / Prepaid Account (CR)', card: 1, span: 2, content: <div className="pxl-readonly-field">{assetAcc ? `${assetAcc.account_code} — ${assetAcc.account_name}` : viewSchedule.asset_account_id}</div> },
          { key: 'expense', label: 'Expense Account (DR)', card: 2, span: 2, content: <div className="pxl-readonly-field">{expAcc ? `${expAcc.account_code} — ${expAcc.account_name}` : viewSchedule.expense_account_id}</div> },
      ...
      <div>
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div className="px-4 py-2.5 border-b border-gray-100">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Amortization Schedule</span>
            </div>
            {entriesLoading ? (
  ...
  if (mode === 'new') {
    ...
    return (
      <LegacyTransactionWorkspace title="Amortization Schedule" family="journal" pattern="D" posting
        ...
        financialFacts={[{ label: 'Schedule Amount', value: fmt(Number(form.total_amount) || 0) }, { label: 'Periods', value: Number(form.total_periods) || 0 }, { label: 'Amount per Period', value: preview == null ? 'Not calculated' : fmt(preview) }]}
        contextFacts={[{ label: 'Schedule', value: form.schedule_name || 'New schedule' }, { label: 'Start Date', value: form.start_date }, { label: 'Asset / Prepaid Account', value: accounts.find(account => account.id === form.asset_account_id)?.account_name || 'Not selected' }, { label: 'Expense Account', value: accounts.find(account => account.id === form.expense_account_id)?.account_name || 'Not selected' }]}
        onBack={() => { setMode('list'); setForm(EMPTY_FORM); setError('') }} backLabel="Amortization Schedules"
        ...
        headerFields={[
          ...
          { key: 'start', label: 'Start Date *', card: 0, content: <input type="date" value={form.start_date} onChange={event => setForm(current => ({ ...current, start_date: event.target.value }))} className="pxl-input w-full" /> },
          { key: 'asset', label: 'Asset / Prepaid Account (CR) *', card: 1, span: 2, content: <select value={form.asset_account_id} onChange={event => setForm(current => ({ ...current, asset_account_id: event.target.value }))} className="pxl-input w-full"><option value="">Select account…</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.account_code} — {account.account_name}</option>)}</select> },
          { key: 'expense', label: 'Expense Account (DR) *', card: 2, span: 2, content: <select value={form.expense_account_id} onChange={event => setForm(current => ({ ...current, expense_account_id: event.target.value }))} className="pxl-input w-full"><option value="">Select account…</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.account_code} — {account.account_name}</option>)}</select> },
  ...
  return (
    <div>
      <div className="pxl-list-header flex items-center gap-2 border-b border-gray-200 px-4 py-2">
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Amortization Schedules</span>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center">
          <p className="text-sm font-medium text-gray-500">No amortization schedules</p>
          <p className="text-xs text-gray-400 mt-1">Create a schedule to auto-generate monthly expense entries from prepaid assets.</p>
        </div>
      ) : (
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {schedules.map(s => {
              ...
              return (
                <tr key={s.id} onClick={() => loadEntries(s)}
