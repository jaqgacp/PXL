        ...
        <div className="flex-1 overflow-auto bg-gray-50 px-5 py-4">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Asset Account (CR)</p>
                <p className="text-gray-700 mt-0.5">{assetAcc ? `${assetAcc.account_code} — ${assetAcc.account_name}` : viewSchedule.asset_account_id}</p>
              </div>
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Expense Account (DR)</p>
                <p className="text-gray-700 mt-0.5">{expAcc ? `${expAcc.account_code} — ${expAcc.account_name}` : viewSchedule.expense_account_id}</p>
              </div>
            </div>
            <div className="mt-3">
      ...
      <div className="flex flex-col h-full">
        <div className="bg-white border-b border-gray-200 px-5 py-2.5 flex items-center gap-2 flex-wrap">
          <button onClick={() => { setMode('list'); setForm(EMPTY_FORM); setError('') }}
            className="text-sm text-gray-500 hover:text-gray-900">← Back</button>
          <span className="text-gray-300">|</span>
          <span className="text-sm font-semibold text-gray-700">New Amortization Schedule</span>
          <div className="ml-auto flex items-center gap-2">
            {error && <span className="text-xs text-red-600 max-w-xs truncate">{error}</span>}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-5 max-w-2xl">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 flex flex-col gap-1">
                ...
                <input value={form.schedule_name} onChange={e => setForm(f => ({ ...f, schedule_name: e.target.value }))} className={inputCls} placeholder="e.g. Prepaid Insurance Jan 2026" />
              </div>
              <div className="sm:col-span-2 flex flex-col gap-1">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Asset / Prepaid Account (CR) *</label>
                <select value={form.asset_account_id} onChange={e => setForm(f => ({ ...f, asset_account_id: e.target.value }))} className={inputCls}>
                  <option value="">— select account —</option>
                  {accounts.map(a => <option key={a.id} value={a.id}>{a.account_code} — {a.account_name}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Expense Account (DR) *</label>
                <select value={form.expense_account_id} onChange={e => setForm(f => ({ ...f, expense_account_id: e.target.value }))} className={inputCls}>
                  <option value="">— select account —</option>
                  {accounts.map(a => <option key={a.id} value={a.id}>{a.account_code} — {a.account_name}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Total Amount *</label>
              ...
              {preview !== null && (
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Monthly Amount Preview</label>
