...
const TABLE_LABELS: Record<string, string> = {
  companies: 'Company', branches: 'Branch', customers: 'Customer', suppliers: 'Supplier', items: 'Item',
  departments: 'Department', cost_centers: 'Cost Center', chart_of_accounts: 'Chart of Accounts',
  payment_terms: 'Payment Term', warehouses: 'Warehouse', employees: 'Employee', bank_accounts: 'Bank Account',
}

export default function CASMasterDataChangeLogPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Master Data Change Log</h1>
        <p className="text-sm text-gray-500 mt-0.5">Change history for customers, suppliers, items &amp; other master records</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap">
        <select value={filterTable} onChange={e => { setFilterTable(e.target.value); setPage(0) }} className="border border-gray-300 rounded-md px-3 py-1.5 text-sm">
          <option value="">All Master Data</option>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">Record Type</th>
            ...
            <tbody>
              {logs.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-16 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'No master data changes recorded.'}</td></tr>
              ) : logs.map(l => (
                <Fragment key={l.id}>
