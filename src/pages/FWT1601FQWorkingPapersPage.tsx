...
export default function FWT1601FQWorkingPapersPage() {
  ...
  const loadHeaders = async () => {
    if (!companyId) return
    setLoading(true)
    const { data } = await supabase.from('compliance_1601fq_working_papers_headers').select('*').eq('company_id', companyId)
  ...
  const loadLines = async (headerId: string): Promise<FormLine[]> => {
    const { data } = await supabase.from('compliance_1601fq_working_papers_lines').select('*').eq('header_id', headerId).order('created_at')
  ...
  const handleSave = async () => {
    ...
    let headerId = editId
    if (!headerId) {
      const { data, error } = await supabase.from('compliance_1601fq_working_papers_headers')
      ...
      const { error } = await supabase.from('compliance_1601fq_working_papers_headers')
    ...
    if (editId) {
      ...
      const { data: dbLines } = await supabase.from('compliance_1601fq_working_papers_lines').select('id').eq('header_id', headerId)
      const toDelete = ((dbLines || []) as { id: string }[]).filter(dl => !keepIds.includes(dl.id)).map(dl => dl.id)
      if (toDelete.length > 0) await supabase.from('compliance_1601fq_working_papers_lines').delete().in('id', toDelete)
    }
    for (const l of lines) {
      if (l.id) await supabase.from('compliance_1601fq_working_papers_lines').update({ reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }).eq('id', l.id)
      else await supabase.from('compliance_1601fq_working_papers_lines').insert([{ header_id: headerId, reference: l.reference || null, amount: l.amount, remarks: l.remarks || null }])
    }
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← 1601FQ Working Papers</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? '1601FQ Working Paper' : editId ? 'Edit Working Paper' : 'New Working Paper'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">Final Withholding Tax — {fmtQuarter(form.period_year, form.period_quarter)}</p>
          </div>
        ...
        <div className={sec}>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-3"><label className={lbl}>Description</label>{isView ? <textarea readOnly value={form.description || '—'} rows={2} className={ro} /> : <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2} placeholder="e.g. 1601FQ Working Paper for Q1 2026" className={inp} />}</div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">1601FQ Working Papers</h1><p className="text-sm text-gray-500 mt-0.5">Quarterly Final Withholding Tax — Schedule &amp; Reconciliation</p></div>
