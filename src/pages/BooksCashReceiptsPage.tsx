...
export default function BooksCashReceiptsPage() {
  ...
  const exportCSV = () => {
    ...
    const csvRows = rows.map(r => [r.date, r.doc_type, r.doc_number, r.payor, r.amount.toFixed(2)])
    const csv = [header, ...csvRows].map(row => row.map(c => `"${c}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `cash-receipts-book-${dateFrom}-to-${dateTo}.csv`; a.click()
