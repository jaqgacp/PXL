...
export default function ITR1701Page() {
  ...
  const handleSave = async () => {
    ...
    if (!editId) {
      ...
      if (error) { alert('Cannot save.\nReason: ' + (error.code === '23505' ? `A 1701 filing for FY ${form.period_year} already exists.` : error.message)); setSaving(false); return }
    } else {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => setMode('list')} className="text-xs text-gray-500 hover:text-gray-900 mb-1">← 1701 Annual ITR</button>
            <h1 className="text-xl font-semibold text-gray-900">{isView ? 'Annual ITR — 1701' : editId ? 'Edit Filing' : 'New Filing'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">FY {form.period_year}</p>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-gray-900">1701 Annual ITR</h1><p className="text-sm text-gray-500 mt-0.5">Annual Income Tax Return — Individual</p></div>
