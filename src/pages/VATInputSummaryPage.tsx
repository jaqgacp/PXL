...
export default function VATInputSummaryPage() {
  ...
  const filtered = rows.filter(r => !search || (r.supplier_name || '').toLowerCase().includes(search.toLowerCase()) || (r.system_no || '').toLowerCase().includes(search.toLowerCase()))

  const totalGross = filtered.reduce((s, r) => s + r.gross_purchases, 0)
  const totalExempt = filtered.reduce((s, r) => s + r.exempt_purchases, 0)
  const totalZero = filtered.reduce((s, r) => s + r.zero_rated, 0)
  ...
  const exportCSV = () => {
    ...
    const csvRows = filtered.map(r => [r.invoice_date, r.source_module, r.system_no || '', r.supplier_name || '', r.supplier_tin || '', r.gross_purchases.toFixed(2), r.exempt_purchases.toFixed(2), r.zero_rated.toFixed(2), r.taxable_base.toFixed(2), r.input_vat.toFixed(2)])
    const csv = [header, ...csvRows].map(row => row.map(c => `"${c}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `input-vat-summary-${MONTHS[selMonth]}-${selYear}.csv`; a.click()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Input VAT Summary</h1>
          <p className="text-sm text-gray-500 mt-0.5">Vendor bills, cash purchases &amp; vendor credits — input VAT by month</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search supplier or doc no..." className="border border-gray-300 rounded-md px-3 py-1.5 text-sm w-56" />
      </div>

      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Gross Purchases</p>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-16 text-gray-400">{!companyId ? 'Select a company from the context bar above.' : 'No purchase transactions in this period.'}</td></tr>
              ) : filtered.map(r => (
                <tr key={r.transaction_id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-right font-mono tabular-nums text-gray-500">{fmt(r.zero_rated)}</td>
