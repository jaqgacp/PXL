    ...
    <div className="mt-3 bg-gray-50 border border-gray-200 rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
        {sections.map(section => (
          <div key={section.key}>
            <div className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-2">{section.title}</div>
            <div className="space-y-2">
              {section.fields.map((f, i) => (
                <div key={i} className={f.wide ? '' : ''}>
                  <div className="text-[10px] uppercase tracking-wide text-gray-400">{f.label}</div>
                  <div className="text-xs font-medium text-gray-800 mt-0.5 break-words">{f.value ?? '—'}</div>
                </div>
              ))}
            </div>
          </div>
