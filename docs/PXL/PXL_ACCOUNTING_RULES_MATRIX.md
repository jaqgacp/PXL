...
| Vendor Bill | Recognizes payable and expense/asset/input VAT; triggered by supplier invoice. | Draft -> approved -> posted -> paid/partially paid -> voided. Approval required where configured. | Post approved VB. | Expense/asset/inventory clearing; Input VAT. | Accounts Payable. | AP from company/supplier; expense/asset from item group/item/document type; input VAT from tax profile/code. | Input VAT; EWT expected/accrual policy must be configurable; FWT if applicable. | AP subledger; inventory matching if linked to RR; FX on settlement if foreign currency. | Supplier, invoice data, items/expenses, tax profile, VAT code, withholding profile, open period, VB number. | VB number; audit approved/posted/voided; related to PO/RR/PV/VC/JE/2307. | Void posts exact reversal and tax counter-rows; posted locked. | Purchase register, AP aging, input VAT, SLP/SLSP, QAP. Tests: EWT policy, VAT, RR linkage, void. |
| Payment Voucher | Records supplier payment; triggered by payment approval/release. | Draft -> posted/released -> cancelled/voided. Approval required by policy. | Post PV/payment. | Accounts Payable. | Cash/bank; EWT payable if withheld. | AP from company/supplier; bank from payment method; EWT from tax profile/ATC. | EWT payable; tax base from withholding policy; 2307 source. | Cash ledger; AP settlement; FX gain/loss if foreign currency. | Supplier, open VB, bank/payment method, withholding profile/ATC, open period, PV number. | PV number; audit posted/cancelled; related to VB/VC/JE/2307. | Cancel posts exact reversal and tax counter-row; lock payment applications. | Cash disbursements, AP aging, EWT summary, QAP, 2307. Tests: line-sum totals, VC-aware over-apply, EWT base/rate. |
...
- CAS/BIR DAT layout, books reconciliation, and exported-byte hashing remain incomplete.
