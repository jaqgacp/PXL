...
| Transaction / audit timeline | Who did what, when | Lifecycle evidence beside the document | CAS user-activity narrative per document | Story view instead of log tables | PXL-DA-016 `transaction_events` (Retested Passed); UI rollout under PXL-AUD-050 | Medium | M | Governed lifecycle stream exists; broad document timeline UI is not wired yet | Phase 2 | Vertical timeline tab: created → approved → posted → voided |
