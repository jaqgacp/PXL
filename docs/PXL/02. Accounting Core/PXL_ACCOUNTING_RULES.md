...
**Status:** Active accounting domain authority
**Authority:** Tier 1 Domain Authority
**Owner / Domain:** Accounting Core
**Applies To:** Accounting Core
**Read When:** Accounting task orientation or accounting-rule conflict
**Do Not Read For:** Detailed posting matrix rows; use PXL_ACCOUNTING_RULES_MATRIX.md
**Last Reviewed:** 2026-07-18 documentation cleanup

Concise accounting reference for PXL. This summarizes standing accounting rules; governed posting behavior now lives in `docs/PXL/02. Accounting Core/PXL_ACCOUNTING_RULES_MATRIX.md`, detailed transaction behavior remains in `docs/PXL/04. Transaction Framework/PXL_TRANSACTION_MATRIX.md`, schema ownership in `docs/PXL/01. Architecture/PXL_SCHEMA_SUMMARY.md`, and open defects live in `docs/PXL/PXL_END_TO_END_AUDIT_FINDINGS.md`.

## Active Production-Readiness Gate

The active milestone is **PXL Accounting Core Ready**, governed by `docs/PXL/02. Accounting Core/PXL_ACCOUNTING_CORE_READINESS.md`.
...
`docs/PXL/02. Accounting Core/PXL_ACCOUNTING_RULES_MATRIX.md` is the official accounting rules matrix. Any new or changed posting behavior must be defined there before implementation.
...
- `docs/PXL/02. Accounting Core/Module Blueprints/01. Journal Entries/` - JE and recurring JE behavior.
- `docs/PXL/02. Accounting Core/Module Blueprints/02. Ledgers/` - GL, account detail, and trial balance behavior.
- `docs/PXL/02. Accounting Core/Module Blueprints/03. Subsidiary Ledgers/` - customer/supplier ledgers and control reconciliation.
- `docs/PXL/02. Accounting Core/Module Blueprints/04. Schedules/` - amortization and revenue recognition schedules.
- `docs/PXL/02. Accounting Core/Module Blueprints/05. Period Management/` - period close, fiscal locks, posting/reversal review, scheduled runs.
- `docs/PXL/02. Accounting Core/PXL_ACCOUNTING_RULES_MATRIX.md` - governed posting rules matrix and account/tax engine architecture.
- `docs/PXL/04. Transaction Framework/PXL_TRANSACTION_MATRIX.md` - per-transaction accounting rules and maturity.
- `docs/PXL/02. Accounting Core/PXL_ACCOUNTING_TEST_BOOK.md` - executable and planned accounting test scenarios.
