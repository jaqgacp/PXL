...
**Last Reviewed:** 2026-08-04 validation and status reconciliation
...
| Preserved module/engine certification status (dashboard only) | `docs/PXL/13. Testing and Validation/PXL_CERTIFICATION_MATRIX.md` |
...
The certification ceremony is retired for automated invariants. The Module and
Engine Certification Standards and the Certification Matrix retain the
certified boundaries already earned; they do not select current work or define
a second phase sequence. Defects remain in
`../PXL_END_TO_END_AUDIT_FINDINGS.md`; the current bounded task and current
counts remain only in `../../AI/AI_STATE.md`. Do not create per-module status
documents or duplicate findings registers.
...
| Regression | `npm run test:db:regression` | Current clean local schema; all **128 pgTAP files / 3,079 assertions** pass with no held-out file. | Mandatory test gate. `npm test` is an alias. |
...
| Canonical dataset | `npm run test:canonical` | Isolated local stack; performs a fresh reset, loads reset/base/enrichment/volume layers atomically, then all **30 canonical files / 751 assertions** pass. | Mandatory canonical gate. This intentionally replaces the local database contents. |
| Frontend source contracts | `npm run test:frontend` | Repository checkout; all **66 Node source-contract tests** pass. | Mandatory final-worktree evidence; separate from the local release-gate composition. |
...
`npm run release:gate:local` composes the fresh-schema, full-regression, canonical, documentation, lint, build, and diff lanes in that order. It is intentionally local-only. Run `npm run test:frontend` alongside it for complete final-worktree evidence. A complete release-candidate decision also requires a successful manual CI dispatch with `release_candidate=true`, which makes both protected hosted lanes mandatory.
...
5. Run frontend source tests, documentation, lint, build, and diff gates, or use `npm run test:frontend` plus `npm run release:gate:local` to execute the complete local final-worktree evidence.
