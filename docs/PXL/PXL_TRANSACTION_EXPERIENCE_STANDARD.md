...
| Activity Timeline | Semantic lifecycle story (created → approved → posted → …) | All | Backed by PXL-DA-016 `transaction_events`; build this as the semantic view over that stream when PXL-AUD-050 reaches the document. Until then, keep `AuditTrailSection` as the visible evidence surface. |
