...
| IA-5 WP-5 detailed event-admission and component-resolution specification (**Engineering Amendment EA-008 COMPLETE 2026-08-01; WP-5 remains unauthorised and unimplemented**; closes WP5-AG-001…003 at specification level through the exact replacement writer/resolver/payload/encoding/security contract, production-versus-rolled-back-fixture boundary, separate trigger-function/object census, totality/current/superseded rules, rollback and future tests `111`…`113`; subordinate to Product Architecture, ADR-C01, ECC-01 and the programme design; ready only for a separate Authorisation Gate re-run) | `07. Inventory/04. Implementation/IA-5_WP-5_DETAILED_EVENT_ADMISSION_AND_COMPONENT_RESOLUTION_SPECIFICATION.md` |
...
| IA-5 ECC hardening implementation design (current-state map, target data model, migration/work-package sequence, test plan; controlling engineering plan — **Work Packages 1–4 certified; WP-5 specification repaired by EA-008 but WP-5 remains unauthorised/unimplemented; WP-5…WP-9 and IA-6 unauthorised; next mission WP-5 Authorisation Gate re-run**) | `07. Inventory/04. Implementation/IA-5_ECC_HARDENING_IMPLEMENTATION_DESIGN_AND_CHANGE_PLAN.md` |
