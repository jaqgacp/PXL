...
| IA-5 WP-5 detailed event-admission and component-resolution specification (**Engineering Amendment EA-009 COMPLETE 2026-08-01; WP-5 remains unauthorised and unimplemented**; preserves EA-008 history and closes the gate re-run findings WP5-AGR-001…003 through certified-WP-4-aligned fourteen-component bytes/digest, one tenant-safe controlling writer sequence, and a fixed reproducible protected-boundary manifest; retains the exact writer/resolver/payload/security, production-versus-rolled-back-fixture, trigger/totality, rollback and future-test contract; subordinate to Product Architecture, ADR-C01, ECC-01 and the programme design; ready only for another separate Authorisation Gate re-run) | `07. Inventory/04. Implementation/IA-5_WP-5_DETAILED_EVENT_ADMISSION_AND_COMPONENT_RESOLUTION_SPECIFICATION.md` |
...
| IA-5 ECC hardening implementation design (current-state map, target data model, migration/work-package sequence, test plan; controlling engineering plan — **Work Packages 1–4 certified; gate re-run rejection and EA-009 closure recorded in §§33–34; WP-5 remains unauthorised/unimplemented; WP-5…WP-9 and IA-6 unauthorised; next mission WP-5 Authorisation Gate re-run**) | `07. Inventory/04. Implementation/IA-5_ECC_HARDENING_IMPLEMENTATION_DESIGN_AND_CHANGE_PLAN.md` |
