...
Of 208 tables, **109 have never held a row** (re-measured 2026-08-05 against the seeded canonical dataset; the close register `fiscal_close_runs` is populated only by the close engine). Opening balances, verified supplier
...
| **Maturity** | **M4 — PARTIAL WORKFLOW** |
| **Certification** | Certification module #4 — **In Progress.** Price-variance policy, supplier-debit integration, broader workflow proof and operated readiness remain open. |
| **Implementation** | Vendor Bill, Cash Purchase, Payment Voucher, Vendor Credit and Purchase Return have governed posting lifecycles. Receiving posts DR Inventory Control / CR Purchase Clearing. Exact PO-line receipt quantity and receipt-linked bill quantity are concurrency-safe; over-receipt and over-billing fail closed. A safe Receiving Report cancellation reverses its exact historical quantity/value for all three costing methods after live bill claims and downstream consumption are corrected. Purchase Return ships the selected receipt layer and completes DR AP / CR Inventory Control at exact inventory cost; the production path is exercised locally. Supplier Debit Memo remains a governed-empty surface, and no price-variance policy has been invented. |
| **Roadmap** | Define price-variance/account-determination policy, integrate the supplier-debit relationship, and prove the complete procure-to-pay/return chain against the module Product Definition of Done. |
...
> **Inventory Accounting Engine** (§4.12) is the separate, dormant IA-5/ECC
> economic-chronology programme. Current-product inventory costing is the
> production authority over stock balances, movements, cost layers and immutable
> allocations described here; it does not activate or consume IA-5/ECC. Never
> quote progress from one as evidence for the other.

| | |
| --- | --- |
| **Purpose** | Know what stock exists, where it is, what it is worth, and why it moved. |
| **Scope** | Stock balances and movements, warehouse-to-warehouse transfers, adjustments, physical counts, inventory valuation, and production cost determination under Weighted Average, FIFO and Specific Identification. Backdated economic replay remains the separate dormant IA-5/ECC scope. |
...
| --- | --- | --- |
| 1 | ~~Receiving increases stock without a journal~~ **CLOSED 2026-08-02 (PXL-AUD-073)** | Inventory now reconciles to its control account at ₱0.00. Exact quantity matching, ordered receipt correction and all three production costing methods are also proven locally as of 2026-08-08. Price variance remains a separate policy decision. |
