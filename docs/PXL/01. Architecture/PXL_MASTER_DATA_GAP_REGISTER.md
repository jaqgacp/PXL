...
| Audit log (`sys_audit_logs`) | Yes | Global | Triggers cover only a subset of masters (see MD-30) |
...
- **Audit coverage:** `sys_audit_logs` triggers cover companies, branches, customers, suppliers, items, and sales documents, but **not COA, tax codes, UOM, payment terms, number series, dimensions, or bank accounts**, so reference/config changes are not captured in the authoritative trail.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| MD-29 | ~~Global `tax_codes` / `vat_codes` / `atc_codes` permit ungoverned authenticated writes~~ **RESOLVED 2026-07-20 (MDP-01 / PXL-AUD-068).** On inspection the raw `USING(true)` hole was already narrowed to admin-gated writes; MDP-01 made the tables read-only and routed all mutations through governed SECURITY DEFINER RPCs with the existing audit trigger (migration `20260721000001`, test 060). | Tax/VAT/ATC codes | **Critical → resolved** | Medium | 1 / 7 | §14, §13 | Done. Residual: restricting to maintainer-only (removing the tenant-admin write path) is a follow-up product decision. |
| MD-01 | No Chart of Accounts template seeded on company creation; new company has an empty COA | Company / COA | High | Large | 1 | §1 | Ship entity-type COA templates and a seed step in company provisioning |
| MD-02 | No automatic fiscal-year / 12-period generation | Fiscal calendar | High | Medium | 1 | §1, §12 | Generate periods on fiscal-year creation with lock controls |
| MD-03 | No automatic number-series provisioning per branch/document type | Number series | High | Medium | 1 | §1 | Provision default series per BIR-registered document type at setup |
...
| MD-09 | COA lacks FS classification / statement grouping (group, subgroup) needed for configurable financial statements | COA | High | Medium | 1 / 8 | §1, §5 | Add FS classification metadata driving FS grouping |
| MD-10 | COA lacks explicit control-account and allow-subledger flags | COA | High | Medium | 1 | §1 | Add flags; reconcile with `company_accounting_config` |
...
| MD-30 | Reference/config master changes (COA, tax codes, UOM, terms, series, dimensions, banks) are not audit-trigger covered | Audit / masters | High | Medium | 1 | §1, §13 | Extend `sys_audit_logs` triggers to remaining masters |
| MD-04 | No default per-company UOM set | UOM | Medium | Small | 1 | §1 | Seed a standard UOM set at provisioning |
| MD-05 | No default per-company EWT/FWT/PT code provisioning | Tax codes | Medium | Medium | 1 / 7 | §1, §14 | Seed common PH withholding/percentage codes at setup |
...
| MD-11 | COA lacks cash-flow classification | COA | Medium | Medium | 1 / 8 | §5 | Add cash-flow category for the cash-flow statement |
| MD-12 | COA lacks tax classification / direct-indirect / capitalizable / opex flags | COA | Medium | Small | 1 | §1 | Add classification flags |
| MD-13 | COA lacks effective/active date window | COA | Medium | Small | 1 | §1 | Add active-from/active-to controls |
