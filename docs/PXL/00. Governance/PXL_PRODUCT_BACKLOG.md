...
| 17 | **Stamp the resolved VAT version on the tax ledger** *(added 2026-08-03)* | ☐ Outstanding. `tax_detail_entries` has `tax_code_id` and `tax_rate` columns that the VAT writers leave NULL; only `vat_code_id` is stored. The exact version *is* recoverable today, because `vat_codes.tax_code_id` and `tax_codes.rate` are both frozen after use, so this is denormalisation for reporting convenience rather than a correctness gap. Doing it touches all eight tax-ledger writers. **Dependency:** none. **Timing:** with the Phase 5 filing work, when a working paper actually needs the rate on the row. |
