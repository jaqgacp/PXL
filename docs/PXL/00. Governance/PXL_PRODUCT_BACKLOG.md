...
| 18e | **Comparative periods and statement notes** *(added 2026-08-03)* | ☐ Outstanding, and **now unblocked** — item 18d shipped, so a prior year's profit sits in Retained Earnings and a comparative across a year boundary finally means something. `fn_financial_statement_report` returns opening/movement/closing for one period; a comparative statement needs the same lines for a prior period side by side, and a signed statement needs note disclosures. The Comparative Financial Statements screen still computes its own figures. **Dependency:** cleared. **Timing:** with the first real statement sign-off. |
